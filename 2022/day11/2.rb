...
class Barrel
  ...
  def start_round
    @monkeys.each do |monkey|
      until monkey.items.empty? do
        ...
        result = monkey.inspect_item(lowest_common_denominator)
  ...
  def lowest_common_denominator
    @lowest_common_denominator ||= @monkeys.map(&:modulo).reduce(&:*)
  end
end


class Monkey
  attr_reader :items, :thrown, :modulo
  ...
  def inspect_item(lowest_common_denominator)
    ...
    new_worry = new_worry % lowest_common_denominator
...
10000.times { |i| barrel.start_round }
