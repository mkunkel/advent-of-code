...
def collect_sizes(hash, values = {}, current_key = nil)
  values[current_key] = 0 unless current_key.nil?
  hash.keys.each do |key|
    if hash[key].is_a?(Hash)
      next_key = [current_key, key].compact.join('->')
      collect_sizes(hash[key], values, next_key)
      values[current_key] += values[next_key] unless current_key.nil?
    elsif current_key
      values[current_key] += hash[key] unless current_key.nil?
    end
  end
  values
end
...
unused_disk = total_disk - dir_sizes['/']
additional_disk_required = required_disk - unused_disk

puts dir_sizes.values.select { |value| value >= additional_disk_required }.min
