...
def coord_list(vent)
  ...
  if x.count == 1 || y.count == 1
    ...
    x_trajectory = vent[:start][0] > vent[:end][0] ? -1 : 1
    y_trajectory = vent[:start][1] > vent[:end][1] ? -1 : 1
    list = [vent[:start]]
    while list.last != vent[:end] do
      last = list.last
      list << [list.last[0] + x_trajectory, list.last[1] + y_trajectory]
    end
    list
  end
end
